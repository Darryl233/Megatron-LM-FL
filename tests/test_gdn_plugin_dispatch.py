...
import torch
from fla_npu.ops.triton import l2norm as ascend_l2norm

import megatron.plugin  # noqa: F401 - loads centralized override registrations
from megatron.core.ssm.gated_delta_net import l2norm
from megatron.plugin.decorators import get_override_method


def test_ascend_gdn_hooks_are_selected():
    l2norm_impl = get_override_method("l2norm.l2norm")
    recurrence_impl = get_override_method("chunk.chunk_gated_delta_rule")
    assert l2norm_impl is ascend_l2norm
    assert recurrence_impl.__module__ == (
        "megatron.plugin.Ascend.ssm.chunk_gated_delta_rule"
    )
